document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('windowSwitchTimes').textContent = `No. of times you have tried to switch window: ${quiz.window_switch_times}`;
    document.getElementById('copyTimes').textContent = `No. of times you have tried to copy texts from the quiz: ${quiz.copied_times}`;
    ...
    (quiz.questions || []).forEach((q, idx) => {
        ...
        (q.options || []).forEach((opt, oidx) => {
            ...
            if (opt.answered && opt.correct) {
                optionContent.style.borderColor = '#22c55e'; // green
                optionContent.style.background = 'rgba(34,197,94,0.08)';
            } else if (opt.answered && !opt.correct) {
                optionContent.style.borderColor = '#ef4444'; // red
