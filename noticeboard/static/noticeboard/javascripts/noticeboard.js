...
imageModal.innerHTML = `
    <div id="modal-content" style="position:relative;max-width:90vw;max-height:90vh;display:flex;align-items:center;justify-content:center;">
        <button id="modal-prev" class="modal-arrow-btn" style="position:absolute;left:-60px;top:50%;transform:translateY(-50%);z-index:2;display:none;">
            <span style="display:inline-block;width:48px;height:48px;border-radius:50%;background:rgba(255,255,255,0.15);box-shadow:0 2px 8px #0004;display:flex;align-items:center;justify-content:center;">
                <i class="fa-solid fa-chevron-left" style="font-size:2rem;color:#fff;"></i>
            </span>
        </button>
        <img id="modal-img" src="" style="max-width:80vw;max-height:80vh;border-radius:12px;box-shadow:0 4px 32px #0008;">
        <button id="modal-next" class="modal-arrow-btn" style="position:absolute;right:-60px;top:50%;transform:translateY(-50%);z-index:2;display:none;">
            <span style="display:inline-block;width:48px;height:48px;border-radius:50%;background:rgba(255,255,255,0.15);box-shadow:0 2px 8px #0004;display:flex;align-items:center;justify-content:center;">
                <i class="fa-solid fa-chevron-right" style="font-size:2rem;color:#fff;"></i>
            </span>
        </button>
        <button id="modal-close" style="position:absolute;top:-40px;right:-40px;background:#fff;border:none;border-radius:50%;width:36px;height:36px;font-size:1.5rem;cursor:pointer;box-shadow:0 2px 8px #0004;z-index:3;">&times;</button>
    </div>
`;
        ...
        postCard.innerHTML = `
            <div class="card-body">
                <div class="d-flex align-items-center mb-2">
                    ...
                    <div>
                        <span class="post-author">${post.anonymous ? 'Anonymous' : post.author}${post.anonymous ? '' : roleLabel}</span><br>
...
if (postInput && postBtn) {
    ...
    postBtn.addEventListener('click', async () => {
        ...
        // Collect files from selectedMedia (which stores {type, url, file})
...
// Update handleMediaUpload to store the File object for upload
function handleMediaUpload(input, type) {
    Array.from(input.files).forEach(file => {
        const url = URL.createObjectURL(file);
        if (!window.selectedMedia) window.selectedMedia = [];
        window.selectedMedia.push({ type, url, file });
...
async function fetchPosts() {
    ...
    try {
        const res = await fetch('/noticeboard/api/posts/list/', { credentials: 'same-origin' });
        const data = await res.json();
        const items = data.results || [];
        if (!items.length) { feed.innerHTML = '<em>No posts yet.</em>'; return; }
        feed.innerHTML = '';
        items.forEach(p => {
            ...
            const role = p.role ? `<span class="badge bg-secondary ms-2">${p.role.charAt(0).toUpperCase() + p.role.slice(1)}</span>` : '';
            const div = document.createElement('div');
            div.className = 'card mb-3';
            div.innerHTML = `
                <div class="card-body">
                    <div class="d-flex align-items-center mb-2">
                        <img src="https://ui-avatars.com/api/?name=${encodeURIComponent(p.username || 'User')}" class="rounded-circle me-2" width="40" height="40" alt="Avatar">
                        <div>
                            <strong>${p.username || 'User'}</strong>${role}<br>
                            <span class="text-muted">${new Date(p.created_at).toLocaleString()}</span>
                        </div>
                    </div>
                    <div class="mb-2">${(notice.caption || '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}</div>
        ...
        feed.innerHTML = '<em>Failed to load.</em>';
    }
}
